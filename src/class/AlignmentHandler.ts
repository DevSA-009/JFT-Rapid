...
class AlignmentHandler {
	 ...
	 * @param params.items - A Selection of PageItems to align.
	...
	static alignPageItemsToArtboard = (
		...
		const { doc, position = "C", objects, engine = "script" } = params;
		...
		const {
			...
			bottom: itemBottom,
			right: itemRight,
		} = Utils.getObjectBounds(objects); // [top, left, bottom, right]
		...
		if (engine === "action") {
			...
			if (objects.length > 1) {
				...
				(tempGroup as PageItem).translate(deltaX, deltaY);
				GroupManager.ungroup(tempGroup);
			} else {
				objects[0].translate(deltaX, deltaY);
			}
		}
	};
